...
export async function renderTiles(root) {
  cleanupRoot(root);
  ...
  const title = document.createElement('h2'); title.textContent = 'Tiles'; header.appendChild(title);
  root.appendChild(header);

  const onScanned = (e) => {
    const d = e && e.detail ? e.detail : null;
    if (!d || d.action !== 'create' || d.view !== 'tiles') return;
    const slid = d.slid ? String(d.slid).trim() : '';
    if (!slid || !/^SLID-/i.test(slid)) return;
    try {
      showTileForm(root, null);
  ...
  const rows = await fetchJson('/api/tiles');
  ...
  if (!(await waitForAgGrid(1200))) {
    const columns = [
      ...
      { key: 'yield_count', label: 'Yield' },
      { key: 'batch_id', label: 'Batch' },
      { key: 'yard_location', label: 'Yard' },
      { key: 'status', label: 'Status' },
      { key: 'qc_status', label: 'QC' },
      // removed stone_type column for tiles
    ];
    const actions = [
      { label: '✏️', title: 'Edit Tile', ariaLabel: 'Edit Tile', className: 'action-btn', onClick: (r) => showTileForm(root, r) },
      { label: '🗑️', title: 'Delete Tile', ariaLabel: 'Delete Tile', className: 'action-btn', onClick: async (r) => { if (!confirm('Delete tile ' + r.tile_id + '?')) return; try { await fetchExpectOk('/tiles/' + encodeURIComponent(r.tile_id), { method: 'DELETE' }); showToast('Deleted tile ' + r.tile_id, 'success'); await renderTiles(root); } catch (e) { showToast('Delete failed: ' + e, 'error'); } } }
    ];
    ...
    const defaultPage = getPersistedPageSize('tiles', 20);
    ...
    const addBtn = document.createElement('button'); addBtn.textContent = 'Create Tile'; addBtn.style.marginLeft = '8px'; addBtn.onclick = () => showTileForm(root, null);
    ...
    pageSizeSel.addEventListener('change', () => { pageSize = Number(pageSizeSel.value || defaultPage); setPersistedPageSize('tiles', pageSize); renderSimple(); });
    btnCsv.onclick = () => {
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'tiles.csv'; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
    };
    btnXlsx.onclick = () => {
      const keys = columns.map(c => c.key).filter(Boolean);
      const filtered = rows.filter(r => { const t = term.toLowerCase(); return !t || Object.keys(r).some(k => String(r[k] ?? '').toLowerCase().includes(t)); }).slice(0, pageSize);
      ...
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Tiles'); XLSX.writeFile(wb, 'tiles.xlsx');
    };
    return;
  }
  ...
  const btnXlsx = document.createElement('button'); btnXlsx.textContent = 'Export Excel'; btnXlsx.style.marginLeft = '8px';
  const pageSizeSel = document.createElement('select'); pageSizeSel.setAttribute('data-pagesize','true'); pageSizeSel.style.marginLeft = '8px';
  const persistedPageSize = getPersistedPageSize('tiles', 20);
  ...
  const columnDefs = [
    { headerName: 'Actions', field: '_actions', pinned: 'left', width: 120, minWidth: 110, maxWidth: 140, suppressHeaderMenuButton: true, sortable: false, filter: false, resizable: false, cellRenderer: (p) => {
      ...
      const edit = document.createElement('button'); edit.type = 'button'; edit.className = 'action-btn'; edit.textContent = '✏️'; edit.title = 'Edit Tile'; edit.setAttribute('aria-label','Edit Tile'); edit.style.marginRight = '6px'; edit.onclick = () => showTileForm(root, p.data);
      const del = document.createElement('button'); del.type = 'button'; del.className = 'action-btn'; del.textContent = '🗑️'; del.title = 'Delete Tile'; del.setAttribute('aria-label','Delete Tile'); del.onclick = async () => { const r = p.data; if (!confirm('Delete tile ' + r.tile_id + '?')) return; try { await fetchExpectOk('/tiles/' + encodeURIComponent(r.tile_id), { method: 'DELETE' }); showToast('Deleted tile ' + r.tile_id, 'success'); await renderTiles(root); } catch (e) { showToast('Delete failed: ' + e, 'error'); } };
    ...
    { headerName: 'Tile ID', field: 'tile_id' },
  ...
  pageSizeSel.addEventListener('change', () => { const v = Number(pageSizeSel.value || persistedPageSize); if (gridApi && gridApi.setGridOption) gridApi.setGridOption('paginationPageSize', v); setPersistedPageSize('tiles', v); });
  btnCsv.onclick = () => { gridApi.exportDataAsCsv({ fileName: 'tiles.csv' }); };
  btnXlsx.onclick = () => { const cols = columnDefs.filter(c => c.field && !c.cellRenderer).map(c => c.field); const out = []; gridApi.forEachNodeAfterFilterAndSort(node => { const obj = {}; cols.forEach(f => { obj[f] = node.data[f]; }); out.push(obj); }); const ws = XLSX.utils.json_to_sheet(out); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Tiles'); XLSX.writeFile(wb, 'tiles.xlsx'); };
}

function showTileForm(root, row) {
  ...
  modalTitle.textContent = row ? `Edit Tile ${row.tile_id}` : 'Create Tile';
  ...
  form.id = 'tile-form-' + Math.random().toString(16).slice(2);
  ...
  if (row) {
    ...
    form.appendChild(formField('yield_count', 'Yield', row.yield_count || '', 'number'));
    form.appendChild(formField('batch_id', 'Batch', row.batch_id || ''));
    form.appendChild(formField('yard_location', 'Yard', row.yard_location || ''));
    // stone_type removed for tiles (server-driven via slab/source)
    ...
    function renderSlabOptions() {
      ...
      const filtered = allSlabs.filter(s => {
        // Strict: only show slabs explicitly marked for 'tiles'
        const st = (s.stone_type || '').toString().trim().toLowerCase();
        if (st !== 'tiles') return false;
        if (!term) return true;
    ...
    form.appendChild(formField('yield_count', 'Yield', '', 'number'));
  ...
  async function closeAndRefresh() {
    ...
    try {
      if (typeof renderTiles === 'function') await renderTiles(root);
      else if (typeof window !== 'undefined' && typeof window.renderTiles === 'function') await window.renderTiles(root);
      else location.hash = '#tiles?' + Date.now();
    } catch (_) { location.hash = '#tiles?' + Date.now(); }
  }
  ...
  async function validateSlidForTile(v) {
    if (!v) { slidNote.textContent = ''; save.disabled = false; return; }
    try {
      // fetch slab details to enforce reserved-family rules
      ...
      if (slabType && ['tiles','cobbles','monuments'].includes(slabType) && slabType !== 'tiles') {
        slidNote.textContent = `SLID ${v} is reserved for ${slabType}; cannot create tiles from it`;
        save.disabled = true;
        return;
      }
      // also ensure SLID hasn't been used by other families
      const u = await fetchSlabUsage(v);
      if ((u.cobbles || 0) > 0 || (u.monuments || 0) > 0) {
  ...
  if (slidInput) { slidInput.addEventListener('change', () => validateSlidForTile(slidInput.value.trim())); slidInput.addEventListener('blur', () => validateSlidForTile(slidInput.value.trim())); }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      ['thickness_mm','length_mm','width_mm','yield_count'].forEach(k => { if (payload[k] === '') delete payload[k]; else payload[k] = Number(payload[k]); });
      // Require block_id or resolve via SLID
      ...
      if (row && row.tile_id) {
        await fetchExpectOk('/tiles/' + encodeURIComponent(row.tile_id), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        showToast('Saved tile ' + row.tile_id, 'success');
      } else {
        await fetchExpectOk('/tiles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        showToast('Created tile', 'success');
      }
      busy.setBusy(false);
      try { dirty.markClean(); } catch (_) {}
      await closeAndRefresh();
  ...
  // place note after SLID input
  const sInput = form.querySelector('input[name="slid"]'); if (sInput && sInput.parentNode) sInput.parentNode.appendChild(slidNote);
  // initial validation when opening form
  if (!row) validateSlidForTile((slidInput && slidInput.value) ? slidInput.value.trim() : '');
}
