...
export async function renderCobbles(root) {
  cleanupRoot(root);
  ...
  const title = document.createElement('h2'); title.textContent = 'Cobbles'; header.appendChild(title);
  root.appendChild(header);

  const onScanned = (e) => {
    const d = e && e.detail ? e.detail : null;
    if (!d || d.action !== 'create' || d.view !== 'cobbles') return;
    const slid = d.slid ? String(d.slid).trim() : '';
    if (!slid || !/^SLID-/i.test(slid)) return;
    try {
      showCobbleForm(root, null);
  ...
  const rows = await fetchJson('/api/cobbles');

  if (!(await waitForAgGrid(1200))) {
    const columns = [
      { key: 'cobble_id', label: 'Cobble ID' },
    ...
    const actions = [
      { label: '✏️', title: 'Edit Cobble', ariaLabel: 'Edit Cobble', className: 'action-btn', onClick: (r) => showCobbleForm(root, r) },
      { label: '🗑️', title: 'Delete Cobble', ariaLabel: 'Delete Cobble', className: 'action-btn', onClick: async (r) => { if (!confirm('Delete cobble ' + r.cobble_id + '?')) return; try { await fetchExpectOk('/cobbles/' + encodeURIComponent(r.cobble_id), { method: 'DELETE' }); showToast('Deleted cobble ' + r.cobble_id, 'success'); await renderCobbles(root); } catch (e) { showToast('Delete failed: ' + e, 'error'); } } }
    ];
    ...
    const defaultPage = getPersistedPageSize('cobbles', 20);
    ...
    const addBtn = document.createElement('button'); addBtn.textContent = 'Create Cobble'; addBtn.style.marginLeft = '8px'; addBtn.onclick = () => showCobbleForm(root, null);
    ...
    addFallbackBanner(root, 'Cobbles');
    ...
    pageSizeSel.addEventListener('change', () => { pageSize = Number(pageSizeSel.value || defaultPage); setPersistedPageSize('cobbles', pageSize); renderSimple(); });
    btnCsv.onclick = () => {
      ...
      const a = document.createElement('a'); a.href = url; a.download = 'cobbles.csv'; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
    };
    btnXlsx.onclick = () => {
      const keys = columns.map(c => c.key).filter(Boolean);
      const filtered = rows.filter(r => { const t = term.toLowerCase(); return !t || Object.keys(r).some(k => String(r[k] ?? '').toLowerCase().includes(t)); }).slice(0, pageSize);
      ...
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Cobbles'); XLSX.writeFile(wb, 'cobbles.xlsx');
    };
    return;
  }
  ...
  const btnXlsx = document.createElement('button'); btnXlsx.textContent = 'Export Excel'; btnXlsx.style.marginLeft = '8px';
  const pageSizeSel = document.createElement('select'); pageSizeSel.setAttribute('data-pagesize','true'); pageSizeSel.style.marginLeft = '8px';
  const persistedPageSize = getPersistedPageSize('cobbles', 20);
  ...
  const columnDefs = [
    { headerName: 'Actions', field: '_actions', pinned: 'left', width: 120, minWidth: 110, maxWidth: 140, suppressHeaderMenuButton: true, sortable: false, filter: false, resizable: false, cellRenderer: (p) => {
      ...
      const edit = document.createElement('button'); edit.type = 'button'; edit.className = 'action-btn'; edit.textContent = '✏️'; edit.title = 'Edit Cobble'; edit.setAttribute('aria-label','Edit Cobble'); edit.style.marginRight = '6px'; edit.onclick = () => showCobbleForm(root, p.data);
      const del = document.createElement('button'); del.type = 'button'; del.className = 'action-btn'; del.textContent = '🗑️'; del.title = 'Delete Cobble'; del.setAttribute('aria-label','Delete Cobble'); del.onclick = async () => { const r = p.data; if (!confirm('Delete cobble ' + r.cobble_id + '?')) return; try { await fetchExpectOk('/cobbles/' + encodeURIComponent(r.cobble_id), { method: 'DELETE' }); showToast('Deleted cobble ' + r.cobble_id, 'success'); await renderCobbles(root); } catch (e) { showToast('Delete failed: ' + e, 'error'); } };
    ...
    { headerName: 'Cobble ID', field: 'cobble_id' },
    ...
    // stone_type column removed for cobbles
  ];
  ...
  pageSizeSel.addEventListener('change', () => { const v = Number(pageSizeSel.value || persistedPageSize); if (gridApi && gridApi.setGridOption) gridApi.setGridOption('paginationPageSize', v); setPersistedPageSize('cobbles', v); });
  btnCsv.onclick = () => { gridApi.exportDataAsCsv({ fileName: 'cobbles.csv' }); };
  btnXlsx.onclick = () => { const cols = columnDefs.filter(c => c.field && !c.cellRenderer).map(c => c.field); const out = []; gridApi.forEachNodeAfterFilterAndSort(node => { const obj = {}; cols.forEach(f => { obj[f] = node.data[f]; }); out.push(obj); }); const ws = XLSX.utils.json_to_sheet(out); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Cobbles'); XLSX.writeFile(wb, 'cobbles.xlsx'); };
}

function showCobbleForm(root, row) {
  ...
  modalTitle.textContent = row ? `Edit Cobble ${row.cobble_id}` : 'Create Cobble';
  ...
  form.id = 'cobble-form-' + Math.random().toString(16).slice(2);
  ...
  if (row) {
    ...
    form.appendChild(formField('shape', 'Shape', row.shape || ''));
    ...
    // stone_type removed for cobbles (server-driven via slab/source)
    ...
    function renderSlabOptions() {
      ...
      const filtered = allSlabs.filter(s => {
        // Strict: only show slabs explicitly marked for 'cobbles'
        const st = (s.stone_type || '').toString().trim().toLowerCase();
        if (st !== 'cobbles') return false;
        if (!term) return true;
    ...
    form.appendChild(formField('shape', 'Shape', ''));
  ...
  async function closeAndRefresh() {
    ...
    try {
      if (typeof renderCobbles === 'function') await renderCobbles(root);
      else if (typeof window !== 'undefined' && typeof window.renderCobbles === 'function') await window.renderCobbles(root);
      else location.hash = '#cobbles?' + Date.now();
    } catch (_) { location.hash = '#cobbles?' + Date.now(); }
  }
  ...
  async function validateSlidForCobble(v) {
    ...
    try {
      ...
      if (slabType && ['tiles','cobbles','monuments'].includes(slabType) && slabType !== 'cobbles') {
        slidNote.textContent = `SLID ${v} is reserved for ${slabType}; cannot create cobbles from it`;
        save.disabled = true;
        return;
      }
      // also ensure SLID hasn't been used by other families
      const u = await fetchSlabUsage(v);
      if ((u.tiles || 0) > 0 || (u.monuments || 0) > 0) {
  ...
  if (slidInput) { slidInput.addEventListener('change', () => validateSlidForCobble(slidInput.value.trim())); slidInput.addEventListener('blur', () => validateSlidForCobble(slidInput.value.trim())); }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      ['length_mm','width_mm','height_mm','pieces_count'].forEach(k => { if (payload[k] === '') delete payload[k]; else payload[k] = Number(payload[k]); });
      ...
      if (row && row.cobble_id) {
        await fetchExpectOk('/cobbles/' + encodeURIComponent(row.cobble_id), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        showToast('Saved cobble ' + row.cobble_id, 'success');
      } else {
        await fetchExpectOk('/cobbles', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        showToast('Created cobble', 'success');
      }
      busy.setBusy(false);
      try { dirty.markClean(); } catch (_) {}
      await closeAndRefresh();
  ...
  if (!row) validateSlidForCobble((slidInput && slidInput.value) ? slidInput.value.trim() : '');
}
